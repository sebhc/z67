...
*********************************************************************
	...
	XTEXT	H67DEF
*
*	Additional definitions for MRX101D interface
*
SECSIZ	EQU	256		Bytes per sector
SPTRK	EQU	40		Sectors per track
TPCYL	EQU	4		Tracks per cylinder
SPCYL	EQU	160		Sectors per cylinder (SPTRK*TPCYL)
TOTCYL	EQU	244		Total cylinders in volume
TOTSEC	EQU	39040		Total # sectors (SPCYL*TOTCYL)
...
MI.OTIR	EQU	263355A		Z80 "OTIR" instruction
MI.INIR	EQU	262355A		Z80 "INIR" instruction
	...
	XTEXT	ESINT
*
*	ORGed high because designed to be launched from menu program
*
	ORG	104176A

START	LXI	H,EXIT
	MVI	A,3
	SCALL	.RENAM		(should be .CTLC? 41Q not 51Q?)
...
*
*	Code was designed to run off floppy disk booted from
*	same drive unit so looks at Boot Device Address.  To run
*	from an explicit port replace next line with MVI A,<port>
*
	LDA	S.BDA			Boot Device Address
	STA	BASE67			store it
	STA	RDDATA+1		port to read data from
	STA	WRDATA+1		port to write data to
	INR	A			control/status is one more than r/w
	STA	WRCTRL+1		port for control status write
	STA	RDSTAT+1		port for reading status
